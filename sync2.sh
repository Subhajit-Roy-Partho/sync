...
# --- Configuration ---
# List of preferred GPUs, from best to worst.
GPUpreference="h100,gh200,l40,a100,a30,ve"

# =========================================================================
# JOB MANAGEMENT FUNCTIONS
# =========================================================================

# ... (Your other functions like updateStatus, jobSubmitter, etc. are unchanged) ...
...
# =========================================================================
# GPU & SUBMISSION LOGIC (Contains the major updates)
# =========================================================================
...
# =========================================================================
# (The rest of your script: updateGpuStatus, the main command parser, etc.)
# ... (These functions are unchanged and should be included here) ...
# =========================================================================

# --- HELPER FUNCTIONS FOR GPU STATUS ---

function _parse_tres_string() { # ... (code from previous answer)
    local tres_string="$1"
...
function _gpu_upsert_row() { # ... (code from previous answer)
    local db_path="$1"; local node_name="$2"; local gpu_type="$3"; local available_count="$4"; local used_count="$5"; local conf_count="$6"
    local sql_command="
    ...
    ON CONFLICT(node, GPU_Type) DO UPDATE SET
        GPU_available = excluded.GPU_available, GPU_used = excluded.GPU_used, GPU_configured = excluded.GPU_configured, last_updated = CURRENT_TIMESTAMP;"
    sqlite3 "$db_path" "$sql_command"
}
function _gpu_cleanup_stale_nodes() { # ... (code from previous answer)
    local db_path="$1"; local operational_nodes_list="$2"; local nodes_in_db; nodes_in_db=$(sqlite3 "$db_path" "SELECT DISTINCT node FROM gpu;")
...
# --- MAIN GPU UPDATE FUNCTION ---

function updateGpuStatus() { # ... (code from previous answer, but remove the echo header lines for quiet mode)
    local operational_nodes; operational_nodes=$(sinfo -N -h -o "%N %T" | grep -v -E 'DOWN|DRAIN|DRNG|FAIL|MAINT|POWER_DOWN|POWER_UP|REBOOT|UNK' | awk '{print $1}' | sort -u)
    if [ -z "$operational_nodes" ]; then sqlite3 "$db_name" "DELETE FROM gpu;"; return 1; fi
    ...
    for node_name in $operational_nodes; do
        ...
        for gpu_type in "${!configured_gpus_map[@]}"; do
            local conf_count=${configured_gpus_map[$gpu_type]}; local used_count=${used_gpus_map[$gpu_type]:-0}
            local available_count=$(( conf_count - used_count )); if [ "$available_count" -lt 0 ]; then available_count=0; fi
...
# (Your main `if [ $# -eq 0 ]` block goes here, unchanged.)
if [ $# -eq 0 ];then
    ...
    echo "-------------------- ------------------------- ---------- ---------- ----------"
    updateGpuStatus # Call the quiet version
    # Now print the results
    sqlite3 -header -column "$db_name" "SELECT node, GPU_Type, GPU_configured, GPU_used, GPU_available FROM gpu WHERE GPU_available > 0 ORDER BY node, GPU_Type;"
    echo "=========================================================================="
    ...
    # submitFirstJob is now more robust.
    if [ $# -eq 4 ]; then
        ...
        echo "Usage: $0 submit <location> [script_name] [resume_script_name]"
    fi
# ... and so on for the rest of your commands
fi