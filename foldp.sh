...
# foldp.sh - Parallel version of fold.sh - Manages batch job submissions with parallel SLURM submissions
# Usage: foldp.sh start $PWD

# Get max number of parallel jobs (number of CPU cores)
MAX_PARALLEL=$(nproc)
...
# Helper function for parallel job submission (called by GNU parallel or xargs)
function submitJobHelper() {
    local location="$1"
    local script="$2"
    local stage="$3"
    local db="$4"
    ...
    local old_jobid=$(sqlite3 "$db" "SELECT jobid FROM jobs WHERE location='$location' AND stage='$stage' AND status=2;")
    ...
    if [[ $newjobid =~ Submitted\ batch\ job\ ([0-9]+) ]]; then
        ...
        sqlite3 "$db" "UPDATE jobs SET jobid='$newjobid', status=2 WHERE location='$location' AND stage='$stage';"
        return 0
    else
...
function submitJob() {
    ...
    submitJobHelper "$location" "$script" "$stage" "$db_name"
}
...
function statusUpdaterParallel() {
    local base_path="$1"

    echo "Updating job statuses in parallel (max $MAX_PARALLEL concurrent)..."

    # Create temporary files for job processing
    local tmp_dir=$(mktemp -d)
    local jobs_file="$tmp_dir/jobs_to_process.txt"
    local pending_file="$tmp_dir/pending_jobs.txt"

    # Get all jobs with job IDs
    sqlite3 -separator '|' "$db_name" "SELECT id, jobid, location, stage, status, max_steps FROM jobs WHERE jobid IS NOT NULL AND jobid != '' AND jobid != 'NULL';" > "$jobs_file"

    # Get pending jobs without job IDs
    sqlite3 -separator '|' "$db_name" "SELECT id, location, stage FROM jobs WHERE status=2 AND (jobid IS NULL OR jobid = '' OR jobid = 'NULL');" > "$pending_file"

    # Process jobs with IDs - check their status
    if [ -s "$jobs_file" ]; then
        echo "Checking status of existing jobs..."

        # Process in parallel using background jobs with semaphore pattern
        local running=0
        while IFS='|' read -r id jobid location stage current_status max_steps; do
            # Skip if any critical field is empty
            if [ -z "$jobid" ] || [ -z "$location" ] || [ -z "$stage" ]; then
                echo "Warning: Skipping job with missing data (id=$id)"
                continue
            fi

            # Run status check in background
            (
                processJobStatus "$base_path" "$id" "$jobid" "$location" "$stage" "$current_status" "$max_steps"
            ) &
    ...
    # Submit pending jobs in parallel
    if [ -s "$pending_file" ]; then
        echo "Submitting pending jobs in parallel..."

        local running=0
        while IFS='|' read -r id location stage; do
            ...
            (
                ...
                submitJobHelper "$location" "$script_name" "$stage" "$db_name"
            ) &
...
function processJobStatus() {
    ...
    1)
        ...
        if checkSimulationCompletion "$location"; then
            ...
            # Calculate progress
            local progress=0
            local max_steps_db=$(sqlite3 "$db_name" "SELECT max_steps FROM jobs WHERE id=$id;")
            if [ -n "$max_steps_db" ] && [ "$max_steps_db" -gt 0 ]; then
                progress=$(calculateProgress "$location" "$max_steps_db")
            fi
            sqlite3 "$db_name" "UPDATE jobs SET progress = $progress WHERE id = $id;"
...
function startWorkflowParallel() {
    local base_path="$1"
    local yaml_file="$base_path/main.yaml"

    echo "Starting workflows for all simulations in parallel (max $MAX_PARALLEL concurrent)..."
    ...
    sqlite3 -separator '|' "$db_name" "SELECT location, stage FROM jobs WHERE stage='$first_stage' AND (jobid IS NULL OR jobid = '');" > "$tmp_file"

    if [ -s "$tmp_file" ]; then
        local total_jobs=$(wc -l < "$tmp_file")
...
function updateJobProgressParallel() {
    local base_path="$1"
    local yaml_file="$base_path/main.yaml"

    echo "Updating job progress in parallel (max $MAX_PARALLEL concurrent)..."

    # Get all running jobs from database (status=2)
    local tmp_file=$(mktemp)
    sqlite3 -separator '|' "$db_name" "SELECT id, location, stage, max_steps FROM jobs WHERE stage IS NOT NULL AND status=2;" > "$tmp_file"
...
# =========================================================================
# DIRECTORY SETUP FUNCTIONS (with parallel processing)
# =========================================================================
...
function setupSimulation() {
    ...
    local sim_folder="$3"
    local sim_name="$4"
    local full_sim_name="$5"
    local output_path="$6"
    local replicas="$7"
    local patterns_str="$8"
    local names_str="$9"
    local copy_all_folder="${10}"

    IFS=',' read -ra patterns_array <<< "$patterns_str"
    ...
    generateJobScripts "$base_path" "" "$replicas" "$full_sim_name" "$sim_output"

    echo "$folders_created"
}
export -f setupSimulation
export -f isJobRunning
export -f generateJobScripts
export -f generateSlurmScript
export -f parseYaml
export -f parseProcedureStage
export -f parseProcedureStageList
export -f getProcedureStages

function setupDirectoryStructureParallel() {
    local base_path="$1"
    local yaml_file="$base_path/main.yaml"
    ...
    echo "Using $MAX_PARALLEL parallel workers for setup..."
    ...
    # Process each folder in the input directory in parallel
    local count=0
    ...
    for sim_folder in "$input_path"/*/; do
        ...
        running=$((running + 1))
        count=$((count + 1))
        total_folders_created=$((total_folders_created + replicas))

        # Limit concurrent setup processes
        if [ $running -ge $MAX_PARALLEL ]; then
            wait -n 2>/dev/null || wait
            running=$((running - 1))
        fi
...
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
    ...
    echo "foldp.sh - Parallel Job management for batch simulations"
    echo ""
    echo "This is the parallel version of fold.sh that uses $MAX_PARALLEL concurrent workers"
    echo ""
    echo "Usage: $0 COMMAND [arguments]"
    echo ""
    echo "Commands:"
    echo "  init                 : Initialize the database for tracking folders and jobs"
    echo "  start <path>         : Initialize directory structure from main.yaml (parallel)"
    echo "  run <path>           : Start workflow for all simulations (parallel job submission)"
    ...
    echo "  update-progress <path> : Update progress for all jobs (parallel)"
    ...
    echo "  status <path>        : Update job statuses and progress stages (parallel)"
    echo ""
    echo "Workflow:"
    echo "  1. $0 init                    # Initialize database"
    echo "  2. $0 start \$PWD              # Setup directories and create scripts (parallel)"
    echo "  3. $0 run \$PWD                # Submit first stage jobs (parallel)"
    echo "  4. $0                         # Update statuses (run periodically, parallel)"
    ...
    echo "  3 = Failed/Other"
    echo ""
    echo "Parallel Workers: $MAX_PARALLEL (based on available CPU cores)"
    exit 0
    ...
    setupDirectoryStructureParallel "$2"
    echo ""
    echo "=========================================================================="
    echo "Setup complete! Now starting workflow..."
    echo "=========================================================================="
    echo ""
    startWorkflowParallel "$2"
    exit 0
    ...
    echo "Checking current job statuses (parallel)..."
    statusUpdaterParallel "$2"
    echo ""
